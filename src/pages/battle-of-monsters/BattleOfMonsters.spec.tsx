...
const battleOfMonstersFactory = async () => {
    mockFetch.mockResponse(req => {
        ...
        if (req.url.includes('battle')) {
            return Promise.resolve(JSON.stringify({ winner: monstersData.monsters[0], tie: false }))
        }
...
describe('BattleOfMonsters', () => {
    ...
    it('should start fight after click on button', async () => {
        ...
        expect(screen.getByTestId("winner-text")).toHaveTextContent(`${monstersData.monsters[0].name} wins`)
    })
})